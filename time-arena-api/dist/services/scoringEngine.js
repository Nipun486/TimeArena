 ...
 */
function getDayFactor(task) {
    if (!task.deadlineDate || !task.endedAt)
        return 1.0;
...
function getActualDaysSpent(task) {
    if (!task.createdAt || !task.endedAt)
        return 0;
...
function calculateScore(task) {
    const { basePoints, difficulty, completionPercentage, actualTimeSpent, estimatedTime, limitType, } = task;
    ...
    const safeCompletion = completionPercentage == null ? 0 : Math.max(0, Math.min(100, completionPercentage));
